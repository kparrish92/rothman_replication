...
questionnaire = read.csv(here("data_g", "data_exp_127819-v11_questionnaire-zbjj.csv"),
                         na.strings = c(""))


## tidy eng lex 

eng_lextale = read.csv(here("data_g", "data_exp_127819-v11_task-fg99.csv"),
                       na.strings = c("")
) %>% 
...
port_lextale = read.csv(here("data_g", "data_exp_127819-v11_task-gdok.csv"),
                        na.strings = c("")) %>% 
  select("Participant.Private.ID",
         "Spreadsheet..targetresponse",
         "Spreadsheet..item",
...
span_lextale = read.csv(here("data_g", "data_exp_127819-v11_task-6d1f.csv"),
                        na.strings = c("")) %>% 
  select("Participant.Private.ID",
         "Spreadsheet..targetresponse",
         "Spreadsheet..item",
...
collocation_task = read.csv(here("data_g", "data_exp_127819-v11_task-t5nj.csv")) %>% 
  select("Participant.Private.ID",
         "Spreadsheet..prompt", 
         "Spreadsheet..correct",
         "Spreadsheet..opt_a",
...
interpretation_task = read.csv(here("data_g", "data_exp_128242-v3_task-6yod.csv")) %>% 
  select("Participant.Private.ID",
         "Spreadsheet..prompt", 
         "Spreadsheet..correct",
         "Spreadsheet..opt_b",
...
add_parts %>% 
  write.csv(here("data_g", "tidy", "add_parts.csv"))


effect_size(2.95,  3.32, 1.64, 1.1) # coll
effect_size(3.01,  3.05, 1.38, 1.09)
